#readind data into r
bikerental<-read.csv(file="day.csv", header=TRUE, sep =",", quote="\"", dec=".",fill=TRUE)
...
a<-qplot(bikerental$actualtemp,bikerental$casual)+geom_smooth(method = 'lm')
b<-qplot(bikerental$actualtemp,bikerental$registered)+geom_smooth(method = 'lm')
a+geom_abline()
b+ geom_abline()
